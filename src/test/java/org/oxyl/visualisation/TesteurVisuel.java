...
public class TesteurVisuel extends JFrame {
	...
	private TesteurRectangleSansPoint testeurRectangle = new TesteurRectangleSansPoint();
	private TesteurCercleSansPoint testeurCercle = new TesteurCercleSansPoint();
	private TesteurTriangleSansPoint testeurTriangle = new TesteurTriangleSansPoint();
	...
	public TesteurVisuel() {
		
		setTitle("Test de mouvement");
		...
		Thread.setDefaultUncaughtExceptionHandler(new UncaughtExceptionHandler() {
			...
			public void uncaughtException(Thread t, Throwable e) {
				if (e.getClass().equals(org.opentest4j.AssertionFailedError.class)) {
					JOptionPane.showMessageDialog(TesteurVisuel.this,
							e.getLocalizedMessage(),
							"Une erreur est survenue.",
							JOptionPane.ERROR_MESSAGE);
				} else {
					e.printStackTrace();
					TesteurVisuel.this.close();
				}
			}
	...
	private Shape initEleveRectangle(Object[] params) {
		return converter.convertRectangleSansPointToShape(testeurRectangle.getRectangle(params));
	}
	
	private Shape initEleveCercle(Object[] params) {
		return converter.convertCercleSansPointToShape(testeurCercle.getCercle(params));
	}
	
	private Shape initEleveTriangle(Object[] params) {
		return converter.convertTriangleSansPointToShape(testeurTriangle.getTriangle(params));
	}
	...
	private Shape translateAndDrawEleve(Shape s, Graphics2D g, double dx, double dy) {
		...
		switch(getCurrentFigure()) {
		case RECTANGLE:
			Object r = converter.convertShapeToRectangleSansPoint(s, 0);
			testeurRectangle.invokeDeplacer(r, dx, dy);
			Shape rectangle = converter.convertRectangleSansPointToShape(r);
			...
			return rectangle;
		case CERCLE:
			Object c = converter.convertShapeToCercleSansPoint(s);
			testeurCercle.invokeDeplacer(c, dx, dy);
			Shape cercle = converter.convertCercleSansPointToShape(c);
			drawingPanelEleve.repaint();
			g.draw(cercle);
			pack();
			return cercle;	
		case TRIANGLE:
			Object t = converter.convertShapeToTriangleSansPoint(s);
			testeurTriangle.invokeDeplacer(t, dx, dy);
			Shape triangle = converter.convertTriangleSansPointToShape(t);
			drawingPanelEleve.repaint();
			g.draw(triangle);
			pack();
			return triangle;
	...
	private Shape rotateAndDrawTemoin(Shape s, Graphics2D g, double theta) {
		...
		f.setToRotation(2*Math.PI*theta/360,x,y);
		Shape shape = f.createTransformedShape(s);
		g.draw(shape);
		pack();
		return shape;
	...
	private Shape rotateAndDrawEleve(Shape s, Graphics2D g, double theta) {
		...
		case RECTANGLE:
			AffineTransform f = new AffineTransform();
			Object r = converter.convertShapeToRectangleSansPoint(s, angleRectangleEleve);
			...
			Shape rectangle = f.createTransformedShape(converter.convertRectangleSansPointToShape(r));
			drawingPanelEleve.repaint();
			g.draw(rectangle);
			pack();
			return rectangle;
	...
	private boolean areCoordinatesEqual(Shape eleve, Shape temoin) {
		Figure f = getCurrentFigure();
		switch (f) {
		case CERCLE :
			Rectangle2D cercleTemoin = temoin.getBounds2D();
			double centreXTemoin = cercleTemoin.getCenterX();
			double centreYTemoin = cercleTemoin.getCenterY();
			double rayonTemoin = cercleTemoin.getHeight()/2.0;
			Object cercleEleve = converter.convertShapeToCercleSansPoint(eleve);
			double centreXEleve = testeurCercle.invokeGetCentreX(cercleEleve);
			double centreYEleve = testeurCercle.invokeGetCentreY(cercleEleve);
			double rayonEleve = testeurCercle.invokeGetRayon(cercleEleve);
			if (Math.abs(centreXTemoin - centreXEleve) > 0.02 || Math.abs(centreYTemoin - centreYEleve) > 0.02 ||
		...
		case RECTANGLE :
			Object rectangleEleve = converter.convertShapeToRectangleSansPoint(eleve, angleRectangleEleve);
			double rcentreXEleve = testeurRectangle.invokeGetCentreX(rectangleEleve);
			double rcentreYEleve = testeurRectangle.invokeGetCentreY(rectangleEleve);
			...
			double rcentreXTemoin = (coordinates[0]+coordinates[2]+coordinates[4]+coordinates[6])/4.0;
			double rcentreYTemoin = (coordinates[1]+coordinates[3]+coordinates[5]+coordinates[7])/4.0;
			...
			if (Math.abs(rcentreXTemoin - rcentreXEleve) > 0.02 || Math.abs(rcentreYTemoin - rcentreYEleve) > 0.02 ||
		...
		case TRIANGLE :
			...
			Object triangleEleve = converter.convertShapeToTriangleSansPoint(eleve);
			double x1Eleve = testeurTriangle.invokeGetX1(triangleEleve);
			double x2Eleve = testeurTriangle.invokeGetX2(triangleEleve);
			double x3Eleve = testeurTriangle.invokeGetX3(triangleEleve);
			double y1Eleve = testeurTriangle.invokeGetY1(triangleEleve);
			double y2Eleve = testeurTriangle.invokeGetY2(triangleEleve);
			double y3Eleve = testeurTriangle.invokeGetY3(triangleEleve);
	...
	private class DrawingPanel extends JPanel {
		...
		private String formatTextBoolean(Shape s) {
			...
			case CERCLE:
				...
				case RECTANGLE :
					if (this.isEleve) {
						Object rectangle = converter.convertShapeToRectangleSansPoint(s, angleRectangleEleve);
				...
				case TRIANGLE : 
					if (this.isEleve) {
						Object triangle = converter.convertShapeToTriangleSansPoint(s);
		...
		private String formatTextCoordinates(Shape s) {
			...
			case CERCLE:
				if (this.isEleve) {
				Object c = converter.convertShapeToCercleSansPoint(s);
				double centreX = testeurCercle.invokeGetCentreX(c);
				double centreY = testeurCercle.invokeGetCentreY(c);
